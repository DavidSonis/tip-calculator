...
/* --------------------------------Step 1: grab the input values------------------------------------------------------ */
  let tipValue = document.getElementById('tipForm').value;
  let billValue = document.getElementById('billInput').value;                                                      //sets a variable for the tip Amount selector to the VALUE of the #tipForm selector
...
/* ---------------------------------------Step 2: turn strings into numbers, use parseFloat() method to do this.-----------  IE: let vairable = parseFloat(stringVariable)----------------------------- */
  let tipAmount = parseFloat(tipValue);
  let billAmount = parseFloat(billValue);                                                        // sets a variable for the Tip Amount that is a number
...
/* --------------------------------Step 3: grab the elements---------------------------------------------------------- */
    ...
    let total = document.getElementById('total-p');                                                         // sets a var to the ID of 'tip-p'
...
/* --------------------------------Step 5: Append your HTML content with the new information-----**use .fixed(2) to display the value with 2 decimal places.-------------------- */
  ...
  bill.innerHTML = "$" + billAmount.toFixed(2);                                                       // change the innterHTML of the bill var from step 3 to the total bill var from step 2
